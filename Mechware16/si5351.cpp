...
const uint8_t SDA_PIN = 31;		// Pin 33
const uint8_t SCL_PIN = 13;		// Pin 32
...
bool Si5351::init(uint8_t xtal_load_c, uint32_t xo_freq, int32_t corr)
{
  ...
  //~ reg_val = Wire.endTransmission();
	...
	bool wr = i2c.transfer((i2c_bus_addr << 1) | I2C_WRITE, 0, 0);
    bool rd = i2c.transfer((i2c_bus_addr << 1) | I2C_READ, 0, 0, I2C_CONTINUE);
    if (rd) {
      uint8_t dummy;
...
uint8_t Si5351::si5351_write_bulk(uint8_t addr, uint8_t bytes, uint8_t *data)
{
	...
	if (!i2c.transfer((i2c_bus_addr << 1) | I2C_WRITE, &addr, 1, I2C_CONTINUE)) {
		 // TODO: Should we abort the transfer somehow?
		 return 1;
	}

	if (!i2c.transferContinue(data, bytes)) {
		return 2;
	}
...
uint8_t Si5351::si5351_write(uint8_t addr, uint8_t data)
{
	...
	if (!i2c.transferContinue(&data, 1)) {
		return 2;
	}
...
uint8_t Si5351::si5351_read(uint8_t addr)
{
	...
	if (!i2c.transfer((i2c_bus_addr << 1) | I2C_WRITE, &addr, 1)) {
		// FIXME
		//~ return false;
	}

	// Read data.
	if (!i2c.transfer((i2c_bus_addr << 1) | I2C_READ, &reg_val, 1)) {
		// FIXME
	}
