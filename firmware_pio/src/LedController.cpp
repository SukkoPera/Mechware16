...
SimpleLedController::SimpleLedController():  lc (PIN_MAX7221_DATA, PIN_MAX7221_CLK, PIN_MAX7221_SEL, 1 /* Number of MAX72xx chips */) {
}

boolean SimpleLedController::begin() {
...
void SimpleLedController::setLedForKey(C16Key key, bool on) {
    if (key != C16Key::NONE) {
...
void SimpleLedController::setAllLeds(bool on) {
    for (byte i = 0; i < MATRIX_COLS; ++i) {
        lc.setRow (0, i, on ? 0xFF : 0x00);
    }
}

void SimpleLedController::setBrightness(byte brightness) {
    lc.setIntensity (0, brightness);
}

// Populates the ledCoordinates array
boolean SimpleLedController::buildLedCoordinates () {
    bool found;
